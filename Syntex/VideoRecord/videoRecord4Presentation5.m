...
% 05-06-07  fwc added trigger counter to output
% 05-06-07  fwc now using 'standard' trigger/response routine
...
% forp=ForpInit;
[trigger response time delay goOn]=ForpCheckChar(forp, -1); % just so that we have used it
...
VideoRecorder('message', sprintf('TriggerNr\tMovieTriggerTime\tExpTriggerTime\tTriggerInterval\tTriggerDelay'));
...
%     [trigger triggerTime triggerDelay goOn]=CheckForTriggerChar(devices, keyNames.triggerKey, quitKey, checkTime);
    [trigger response triggerTime triggerDelay goOn]=ForpCheckChar(forp, checkTime);
    if goOn==0
    ...
    if trigger==1
        ...
        VideoRecorder('message', sprintf('%d\t%.2f\t%.2f\t%.2f\t%d', triggerCounter, tt, expTT, deltaTT, round(triggerDelay*1000) ));

        Screen('FillRect', window, gray, [0 0 h 60]);
        DrawFormattedTextOnPoint(window, ['Tt: ' sprintf('%d\t%.2f', triggerCounter, tt)], 400, 20, white, 'center', 'center');
        doFlip=1;
    end
