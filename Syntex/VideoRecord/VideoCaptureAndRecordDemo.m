...
try
    ...
    while (GetSecs - t) < 600
        ...
        [tex(txc) pts nrdropped]=Screen('GetCapturedImage', win, grabber, 1);
        ...
        if (tex(txc)>0)
            ...
            Screen('DrawTexture', win, tex(txc), [], CenterRect(Screen('Rect', win)/2, Screen('Rect', win)));

            Screen('glPopMatrix', win);

            % Print pts:
            Screen('DrawText', win, sprintf('%d', txc), 0, 0, white);
    ...
    disp('now saving to avi video');
    matrix2avi(vimat,'file','myvideo.avi','fps',round(avgfps),'map', 'gray');
