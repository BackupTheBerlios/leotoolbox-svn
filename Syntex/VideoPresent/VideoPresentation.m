...
% may 07    fwc added video recording as option
...
sliderInstr = 'No instruction'; % instruction now get's taken from the par file
...
try
    ...
    if 1==videoRecMode
        ...
        remoteVideoControl( logVideoFrameMode );   
    end
    ...
    if 1==videoRecMode
        ...
        remoteVideoControl('message', ['MovieRecStartTime:' sprintf('%.2f', movieStartTime ) ] );
    end
...
%                 Screen('DrawTexture', window, tex);
                ...
                if first==1
                    ...
                    first=0;
                    if 1==videoRecMode
                        remoteVideoControl('message', ['MovieStart:' sprintf('%.2f', vbl-movieStartTime ) ] );
                    end
                end
                % Release texture:
                Screen('Close', tex);
        ...
        if 1==videoRecMode
            remoteVideoControl('message', ['MovieEnd:' sprintf('%.2f', stimulusOffsetTime-movieStartTime ) ] );
            remoteVideoControl('message', ['ActualMovieDuration:' sprintf('%.2f', actStimDur ) ] );
        end
    ...
    if 1==videoRecMode
        remoteVideoControl('message', ['End of experiment:' sprintf('%.2f', GetSecs-movieStartTime ) ] );
        WaitSecs(2);
        disp('stop recording');
        remoteVideoControl('stoprecording');
        WaitSecs(2);
        disp('shutdown video recorder');
