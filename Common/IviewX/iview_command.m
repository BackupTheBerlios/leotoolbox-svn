function[ command ] = iview_command( name, parameters )
...
switch( name ) 
    ...
    case 'ET_FIN'  
        ep = 0; % sent when cal. has finished successfully
    case 'ET_FRM'
        % data format
    case 'ET_IMG'
    case 'ET_INC' 
    ...
    case 'ET_LNS' %
        'ET_MOV' %
        'ET_PNT' %s'ET cal.point 'ET_pnt 1 400 300
        'ET_PSE' %pause current recording
        'ET_RCL' %drift correction start
        'ET_REC' %start rectording optional time 'ET_rec 5 = secondes
        'ET_REM' %remark in data
        'ET_SAV' %svave data to file : 'ET_sav "c:\t.idf"
        'ET_SFT' %remotely control tracker param'ETers : eye type (0 left/1 right), param'ETer type
        % 0 = pupil threshold
        % 1 = reflex threshold
        % 2 = show aio
        % 3 = show contour
