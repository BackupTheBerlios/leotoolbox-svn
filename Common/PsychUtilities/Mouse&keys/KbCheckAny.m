function [keyIsDown,secs, keyCode] = KbCheckAny(deviceNumber)
% [keyIsDown, secs, keyCode] = KbCheckAny([deviceNumber])
%
...
% 140507    fwc added code for checking multiple devices
...
if macosx
    if nargin==1
        [keyIsDown, secs, keyCode]= PsychHID('KbCheck', deviceNumber(1));
        if length(deviceNumber)>1
            for k=2:length(deviceNumber)
                [kd,s,kc]= PsychHID('KbCheck', deviceNumber(k));
