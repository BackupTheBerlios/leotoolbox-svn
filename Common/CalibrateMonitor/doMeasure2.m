...
for measureNum = 1:nrTrials
	...
	if par.meas.simulate ~= 1
        ...
        fprintf('Measuring\n');
        fprintf(' attempt #1');
        while isempty(strfind(buffer,'OK')) && (timePast < 30)
            timePast = toc;
            fprintf('.');
            if (timePast > 10 && attempt == 1) || (timePast > 20 && attempt == 2)
                % resend MES-command
                attempt = attempt+1;
                fprintf('\n attempt #%d',attempt);
                SerialComm('write',par.cm.port,['MES',13,10,0]); % write for measurement
            end
            waitSecs(1);  % give Minolta some time to do its things
        ...
        if (length(commas)>0)
            ...
            % better save the measurement vec every time as a backup?!
