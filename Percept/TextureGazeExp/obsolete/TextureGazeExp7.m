...
% try
    ...
    fprintf(fp, 'SUBJECT\tSESSION\tROW\tTRIAL\tDATE\tTIME\tDELAY\tACTSTIMDUR\tTTEXTURE\tTTEXTANGLE\tTPOS\tCTEXTURE\tCTEXTANGLE\tCHOICE\tRESPONSE\tLAT\n');
    fclose(fp);
...
%         window
        [imgfiles, tpos, nStim, xpos, ypos, gapangle, texAngle]=createImageStimulus(window, targetDir, distrDir, par.nrStimuli(row), par.stimRadius(row), par.stimSize(row), par.stimOrient(row), par.maskSD(row));
        % add fixationPoint
        ...
        Eyelink('message', 'RESPONSE %s', tpos==choice );
...
function [myimgfile, tpos, nStim, xpos, ypos, gapangle, texAngle]=createImageStimulus(window, targetDir, distrDir, nStim, radius, siz, ori, masksd )
