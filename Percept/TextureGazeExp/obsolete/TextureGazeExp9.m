...
dummymode=1;
autorun=1;   % no waiting for fixation
...
%         window
        ...
        actGapDur=stimulusOnsetTime-gapOnsetTime;
        ...
        Screen('Close', oswin); % this will close offscreen window
        ...
        if goOn==1
            ...
            fprintf(fp, '%s\t%.1f\t%d\t%d\t%.1f\n', imf, ta, choice, (tpos==choice), latency*1000);
            fclose(fp);
            ...
            fprintf(fp, 'TIME\t%s\n', time);
            fprintf(fp, 'STIMULUS\n');
            fprintf(fp, 'XPOS\tYPOS\tSIZE\tORIENT\tGAPANGLE\tISTARGETPOS\tISCHOICE\tIMAGE\n');
            for i=1:length(imgfiles)
                fprintf(fp, '%d\t%d\t%d\t%.1f\t%d\t%d\t%s\n', xpos(i), ypos(i), -999, texAngle(i), gapangle(i), (i==tpos), (i==choice), imgfiles{i} );
            end
            fprintf(fp, 'DELAY\t%.f\n', actDelayDur*1000);
            fprintf(fp, 'DELAY\t%.f\n', actGapDur*1000);
            fprintf(fp, 'ACTSTIMDUR\t%.f\n', actStimDur*1000);
            fprintf(fp, 'TARGET_TEXTURE\t%s\n', imf);
            fprintf(fp, 'TARGET_TEXTURE_ANGLE\t%f\n', ta);
            fprintf(fp, 'TARGET_NR\t%d\n', choice);
            fprintf(fp, 'RESPONSE\t%d\n', (tpos==choice));
            %             fprintf(fp, 'RT\t%.1f\n', rt*1000);
