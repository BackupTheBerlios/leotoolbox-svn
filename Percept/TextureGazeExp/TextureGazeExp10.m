...
disp(edfFile);
    ...
    enforceDriftCorr=50; % amount of trials after which we enforce a driftcorrection
...
%         window
        ...
        if goOn==1
            ...
            fprintf(fp, 'TIME\t%s\n', time);
            fprintf(fp, 'STIMULUS\n');
            fprintf(fp, 'XPOS\tYPOS\tORIENT\tISTARGETPOS\tISCHOICE\tIMAGE\n');
            for i=1:length(imgfiles)
                fprintf(fp, '%d\t%d\t%.1f\t%d\t%d\t%s\n', xpos(i), ypos(i), texAngle(i), (i==tpos), (i==choice), imgfiles{i} );
            end
            fprintf(fp, 'DELAY\t%.f\n', actDelayDur*1000);
            fprintf(fp, 'DELAY\t%.f\n', actGapDur*1000);
