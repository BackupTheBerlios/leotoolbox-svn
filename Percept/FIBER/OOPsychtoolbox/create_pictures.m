function[pictures] = create_pictures(bitmaps, run, savefile)
%
...
ws = load(filename);
ordering = designmatrix(number_per_picture, [number_per_picture/2 number_per_picture/2]);

save(savefile, 'ordering');

fprintf('design saved to : %s\n', savefile);

% if else
%     if (psl == 'h')
%         filename = ['houses_only'];
%         ws = load(filename);
%         ordering = ws.ws.d;
% 
%     else
...
for i = 1:60
    ...
    for j = 1:number_per_picture,
        ...
        image = bitmaps(o);
        p = add(p, 'bitmap', image, 'location',locations{j} , 'position', 'rel', 'elementtype', type);
        % p = add(p, 'text', '+', 'location', [512 384]);
    end;
    pictures(index) = p;
    index =index +1;
